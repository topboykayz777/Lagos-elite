...
import { Phone, Menu, X, ChevronDown, Building2, MapPin, ShieldCheck, TrendingUp, Key } from 'lucide-react';
          ...
          <NavigationMenu>
            <NavigationMenuList className="gap-2">
              ...
              <NavigationMenuItem>
                ...
                <NavigationMenuContent>
                  <ul className="grid w-[500px] gap-3 p-6 md:grid-cols-2 bg-white border-t-4 border-[#C5A059]">
                    {propertyLinks.map((link) => (
                      <li key={link.title}>
                        <NavigationMenuLink asChild>
                          <Link
                            href={link.href}
                            className="block select-none space-y-1 p-3 leading-none no-underline outline-none transition-colors hover:bg-zinc-50 group"
                          >
                            <div className="flex items-center gap-2">
                              <link.icon className="w-4 h-4 text-[#C5A059]" />
                              <div className="text-xs font-black uppercase tracking-widest text-[#002147] group-hover:text-[#C5A059]">
                                {link.title}
                              </div>
                            </div>
                            <p className="line-clamp-2 text-[10px] leading-snug text-zinc-500 font-medium">
                              {link.desc}
                            </p>
                          </Link>
                        </NavigationMenuLink>
                    ...
                    <li className="col-span-2 mt-2 pt-4 border-t border-zinc-100">
                      <Link href="/properties" className="text-[10px] font-black uppercase tracking-[0.2em] text-[#C5A059] hover:underline flex items-center justify-center gap-2">
                        View All Listings <ChevronDown className="w-3 h-3 -rotate-90" />
                      </Link>
                    </li>
                  </ul>
                </NavigationMenuContent>
              ...
              <NavigationMenuItem>
                ...
                <NavigationMenuContent>
                  <ul className="grid w-[200px] gap-2 p-4 bg-white border-t-4 border-[#C5A059]">
