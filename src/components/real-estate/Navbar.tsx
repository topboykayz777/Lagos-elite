      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2">
          ...
          <div className="flex flex-col leading-none">
            <span className={cn("text-xl font-bold tracking-tighter", isScrolled ? "text-[#002147]" : "text-white")}>
              LAGOS<span className="text-[#C5A059]">ELITE</span>
            </span>
            <span className={cn("text-[10px] uppercase tracking-[0.2em] font-medium", isScrolled ? "text-zinc-500" : "text-zinc-300")}>
              Properties
            </span>
          </div>
        </Link>
        ...
        >
          {isMobileMenuOpen ? (
            <X className={cn("w-8 h-8", isScrolled ? "text-[#002147]" : "text-white")} />
          ) : (
            <Menu className={cn("w-8 h-8", isScrolled ? "text-[#002147]" : "text-white")} />
          )}
        </button>
      </div>
